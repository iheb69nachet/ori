...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function filterProducts(array $filters, array $params = [])
    {
        ...
        // dd($filters['why']);
        if(isset($filters['why'])){

            $filters['why'] = array_filter($filters['why']);
            if ($filters['why']) {
                $this->model = $this->model
                    ->where(function ($query) use ($filters) {
                        foreach ($filters['why'] as $brandId) {
                            $query->OrWhereRaw("FIND_IN_SET(?, ec_products.why)", [$brandId]);
                        }
                    });
            }
        }
        if(isset($filters['when'])){
            ...
            $filters['when'] = array_filter($filters['when']);
            if ($filters['when']) {
                $this->model = $this->model
                    ->where(function ($query) use ($filters) {
                        foreach ($filters['when'] as $brandId) {
                            $query->OrWhereRaw("FIND_IN_SET(?, ec_products.when)", [$brandId]);
                        }
                    });
            }
        }
        if(isset($filters['where'])){
            ...
            $filters['where'] = array_filter($filters['where']);
            if ($filters['where']) {
                $this->model = $this->model
                    ->where(function ($query) use ($filters) {
                        foreach ($filters['where'] as $brandId) {
                            $query->OrWhereRaw("FIND_IN_SET(?, ec_products.where)", [$brandId]);
                        }
                    });
            }
        }
        if(isset($filters['how'])){

            $filters['how'] = array_filter($filters['how']);
            if ($filters['how']) {
                $this->model = $this->model
                    ->where(function ($query) use ($filters) {
                        foreach ($filters['how'] as $brandId) {
                            $query->OrWhereRaw("FIND_IN_SET(?, ec_products.how)", [$brandId]);
                        }
                    });
            }
        }
        if(isset($filters['who'])){
            ...
            $filters['who'] = array_filter($filters['who']);
            if ($filters['who']) {
                $this->model = $this->model
                    ->where(function ($query) use ($filters) {
                        foreach ($filters['who'] as $brandId) {
                            $query->OrWhereRaw("FIND_IN_SET(?, ec_products.who)", [$brandId]);
                        }
                    });
            }
        }
        if(isset($filters['what'])){

            $filters['what'] = array_filter($filters['what']);
            if ($filters['what']) {
                $this->model = $this->model
                    ->where(function ($query) use ($filters) {
                        foreach ($filters['what'] as $brandId) {
                            $query->OrWhereRaw("FIND_IN_SET(?, ec_products.what)", [$brandId]);
                        }
                    });
            }
        }
        if(isset($filters['how_many'])){

            $filters['how_many'] = array_filter($filters['how_many']);
            if ($filters['how_many']) {
                $this->model = $this->model
                    ->where(function ($query) use ($filters) {
                        foreach ($filters['how_many'] as $brandId) {
                            $query->OrWhereRaw("FIND_IN_SET(?, ec_products.how_many)", [$brandId]);
                        }
                    });
            }
        }
